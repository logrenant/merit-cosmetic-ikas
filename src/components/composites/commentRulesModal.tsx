...
const CommentRulesModal: React.FC<CommentRulesModalProps> = ({ trigger, rulesHtml, modalTitle, commentRules, buttonText }) => {
    ...
    return (
        <>
            ...
            {showModal && (
                <>
                    <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-hidden focus:outline-hidden mx-4">
                        <div className="relative w-auto my-6 mx-auto max-w-2xl">
                            <div
                                ref={ref}
                                className="border-0 p-6 gap-3 rounded-sm shadow-lg relative flex flex-col w-full bg-[color:var(--bg-color)] outline-hidden focus:outline-hidden max-h-[80vh] overflow-y-auto"
                            >
                                {Boolean(modalTitle) && (
